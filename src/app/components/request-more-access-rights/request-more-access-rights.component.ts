...
@Component({
  selector: 'app-request-more-access-rights',
  templateUrl: './request-more-access-rights.component.html',
  styleUrls: ['./request-more-access-rights.component.css']
})
export class RequestMoreAccessRightsComponent implements OnInit {
  ...
  async createAuthPayload(read, create, update, delet) {
    ...
    this.authData = {
      patient: this.childMessage.authID,
      accessRights: [],
      type: 'more access'
    };


    if (read) { this.authData.accessRights.push('Read'); }
    if (create) { this.authData.accessRights.push('Create'); }
    if (update) { this.authData.accessRights.push('Update'); }
    if (delet) { this.authData.accessRights.push('Delete'); }
